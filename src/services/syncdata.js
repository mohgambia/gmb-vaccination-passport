...
export const retrieveData = async (patientId, callback,date, name, lastName) => {
  let link = !patientId ? `http://draman.iprocuratio.com/api/patients/checkPatient?firstName=${name}&lastName=${lastName}&birthDate=${date}` : `https://draman.iprocuratio.com/api/patients/${patientId}`
